...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = offer.offerProductId
      ? await dissolveOfferInventoryProduct({ offerId: id, actorId: session.userId, mode: payload.mode, quantity: payload.quantity, note: payload.note, idempotencyKey: payload.idempotencyKey }, db)
      : await dissolveStoreOfferBundle({ offerId: id, actorId: session.userId, mode: payload.mode, quantity: payload.quantity, note: payload.note, idempotencyKey: payload.idempotencyKey });
    await writeAuditLog({ actorId: session.userId, action: "update", entityType: "admin_store_offer_bundle_dissolve", entityId: id, afterData: result });
